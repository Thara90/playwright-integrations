...
test('Login with valid credenatials', async ({ request }) => {
  ...
  const _response = await userClient.createLogin(ApiTestInputData.credentials.valid);
...
test('Login with invalid username and valid password', async ({ request }) => {
  const userClient = new UserClient(request);
  const _response = await userClient.createLogin(ApiTestInputData.credentials.invalidUsername);
...
test('Login with valid username and invalid password', async ({ request }) => {
  const userClient = new UserClient(request);
  const _response = await userClient.createLogin(ApiTestInputData.credentials.invalidPassword);
  expect.soft(_response.status()).toBe(401);
